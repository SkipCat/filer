		...
		if (!empty($_FILES['userfile']['name'])) {
			...
			*/
			if (file_check_upload($_FILES)) {
				file_upload($_FILES);
				write_log('access.log', 'File ' . $_FILES['userfile']['name'] . ' uploaded.');
			...
			write_log('security.log', 'No file to upload.');
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if (!empty($_POST['filename'])) {
			if (file_check_permission()) {
				file_rename($_POST);
				write_log('access.log', 'File ' . $_POST['input-filename'] . ' renamed.');
				header('Location: ?action=home');
				exit(0);
			}
			else {
			...
			write_log('security.log', 'No name to rename file.');
...
function replace_action() {
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (!empty($_FILES['filename']['name'])) {
			if (file_check_permission()) {
				file_replace($_POST);
				write_log('access.log', 'File ' . $_POST['input-filename'] . ' replaced.');
				header('Location: ?action=home');
				exit(0);
			}
			else {
			...
			write_log('security.log', 'No file to replace file.');
			echo "<p style='color:white;font-family:Calibri'>" . "Choisissez un fichier." . "</p>";
		}
	}
	require('views/home.html');
...
function delete_action() {
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (file_check_permission()) {
			file_delete($_POST);
			write_log('access.log', 'File ' . $_POST['input-filename'] . ' deleted.');
			header('Location: ?action=home');
			exit(0);
		}
		else {
...
function modify_action() {
	if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
		if (!empty($_POST['content-modification'])) {
			if (file_check_permission()) {
				file_modify($_POST);
				write_log('access.log', 'File ' . $_POST['input-filename'] . ' modified.');
				header('Location: ?action=home');
				exit(0);
			}
			else {
			...
			write_log('security.log', 'No content to modify file.');
...
function move_action() {
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if (!empty($_POST['new-folder'])) {
			if (file_check_permission()) {
				file_move($_POST);
				write_log('access.log', 'File ' . $_POST['input-filename'] . ' moved.');
				header('Location: ?action=home');
				exit(0);
			}
			else {
			...
			write_log('security.log', 'No folder to move file.');
			echo "<p style='color:white;font-family:Calibri'>" . "Choisissez un nom." . "</p>";
		}
	}
	require('views/home.html');
